...
ruby "${MARSYAS_RUBY}"/roc-auc.rb "${VALIDATION_TXT}" "${STACKING}"stage1_predictions.txt > "${STACKING}"stage1_evaluation.txt
ruby "${MARSYAS_RUBY}"/roc-auc.rb "${VALIDATION_TXT}" "${STACKING}"stage2_predictions.txt > "${STACKING}"stage2_evaluation.txt
